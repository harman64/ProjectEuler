...
const largestProductGrid = () => {
    ...
    grid = grid.split('\n    ');
    grid = grid.map(elem => elem.split(" ").map(str => parseInt(str)));

    let maxProduct = grid[0][0]*grid[0][1]*grid[0][2]*grid[0][3];

    for(let i=0;i<17;i++){
        for(let j=0;j<17;j++){
            let product4;
            let product1 = grid[i][j]*grid[i][j+1]*grid[i][j+2]*grid[i][j+3];
            let product2 = grid[j][i]*grid[j+1][i]*grid[j+2][i]*grid[j+3][i];
            let product3 = grid[i][j]*grid[i+1][j+1]*grid[i+2][j+2]*grid[i+3][j+3];
            let product = product1>product2?product1:product2;
            product =  product>product3?product:product3;
            if(i-3>=0 && j+3<=19){
                product4 =  grid[i][j]*grid[i-1][j+1]*grid[i-2][j+2]*grid[i-3][j+3];
                product = product>product4?product:product4;
            }
            maxProduct = maxProduct>product?maxProduct:product;
        }
    }
    console.log('Problem 11: Largest Product Problem:',maxProduct);
}
...
const triangularNos = () => {
    ...
    while(true){
        triangularNo +=i;
        i +=1;
        if(noOfDivisors(triangularNo)>500){
            break;
        }
    }

    console.log("Problem 12: Highly divisible triangular number:",triangularNo);
}
...
const largeSum = () => {
    const template = `37107287533902102798797998220837590246510135740250
    ...
    let nosArr = template.split('\n    ');
    nosArr = nosArr.map( elem => elem.split("").map(arrElem => parseInt(arrElem)));
    let finalSum="";
    let carry =0;
    for(j=49;j>=0;j--){
        let sum = carry;
        ...
        //finalSum +=(j===0?sum:(sum%10)); 
    }

    finalSum = "" + carry +finalSum.split("").reverse().join("");

    console.log("Problem 13: Large Sum:", finalSum.substr(0,10));
